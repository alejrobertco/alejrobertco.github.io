...
const cardpuntaje = document.getElementById("puntaje");                              //aqui se define la tarjeta

const GANARb = document.getElementById("GANARb");
const GANARc = document.getElementById("GANARc");

const GANARd = document.getElementById("GANARd");
...
class PINCHOSS {
    ...
    CONSTRUIR() {
        ctx.fillStyle = this.color;
        ctx.beginPath();
        ctx.moveTo(this.x +10,this.y +30,this.size,this.size);
        ctx.lineTo(this.x +30,this.y -40,this.size,this.size);
        ctx.lineTo(this.x +40,this.y -50,this.size,this.size);
        ctx.lineTo(this.x +50,this.y -40,this.size,this.size);  // la y es el punto de arriba
        ctx.lineTo(this.x +50,this.y +30,this.size,this.size); // la y es el punto de abajo
        ctx.closePath(); 
...
function crearBloques() {
    ...
    pinchos.push(new PINCHOSS(30, velocidadPinchos));     //PARA PONERLOS BAJO EL SUELO-----------------------------------------------------
...
function RAPIDEZ() {                                                          //FUNCION PARA LLEGAR A LOS PUNTOS REQUERIDOS

        if(puntajeIncremento + 10 === puntaje){
            GANARc.textContent = puntaje;
            GANARb.style.display = "block";
            GANARd.style.display = "block";
