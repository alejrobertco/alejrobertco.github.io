...
class Player {
    ...
    contarROTACION() {
        ...
        let offsetYPosition = this.y + (this.size / 2);
        ctx.translate(offsetXPosition,offsetYPosition);
        ctx.rotate(-this.spin * Math.PI / 179.9 );
        ctx.translate(-offsetXPosition,-offsetYPosition);
    }
...
class PINCHOSS {
    ...
    CONSTRUIR() {
        ctx.fillStyle = this.color;
        ctx.beginPath();
        ctx.moveTo(this.x +0,this.y +30,this.size,this.size);
        ctx.lineTo(this.x +30,this.y -40,this.size,this.size);  // la y es el punto de arriba
        ctx.lineTo(this.x +60,this.y +30,this.size,this.size); // la y es el punto de abajo
        ctx.closePath(); 
...
function RAPIDEZ() {                                                          //FUNCION PARA LLEGAR A LOS PUNTOS REQUERIDOS
        ...
        if(puntajeIncremento + 15 === puntaje){
